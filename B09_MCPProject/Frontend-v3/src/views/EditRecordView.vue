...
onMounted(async () => {
  try {
    ...
    try {
      const fetched = await getRecordsByStudent(String(studentResult.id))
...
function addRecord() {
  if (!student.value) {
    message.value = 'Cannot add record: missing student information.'
  ...
  records.value.push({
    ...
    /**
     * 修改点：
     * 保存 record 时 studentId 使用 String(student.id)。
     */
    studentId: String(student.value.id),
...
async function saveRecords() {
  ...
  if (!student.value) {
    message.value = 'Save failed: missing student information.'
    isMsgError.value = true
    return
  }
  ...
  try {
    for (const r of records.value) {
      /**
       * 修改点：
       * 保存前强制 studentId 等于当前学生 ID。
       */
      r.studentId = String(student.value.id)
      await saveRecord(r)
    }
