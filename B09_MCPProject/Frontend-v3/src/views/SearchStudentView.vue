...
async function searchStudent() {
  ...
  try {
     ...
     *
     * 具体实现写在 org.ts 的 searchStudentById() 里。
     */
    const studentResult = await searchStudentById(input)
    ...
    try {
      /**
       * 修改点：
       * student.id 可能是 number，也可能是 string。
       * records 接口统一传 String。
       */
      interviewRecords.value = await getRecordsByStudent(String(studentResult.id))
    } catch (recordErr: any) {
...
function editRecord() {
   ...
   * Edit 页面传 student.id。
   */
  router.push(`/edit-record/${encodeURIComponent(String(foundStudent.value.id))}`)
}
